...
export function Editor({
  ...
  return (
    <div className="w-150 h-200 bg-white flex flex-col rounded-lg shadow-lg relative px-6">
      <div
        className="flex bg-gray-200 rounded-2xl p-2 w-fit mt-5 cursor-pointer"
        onClick={() => navigate(-1)}
      >
        <Undo2 />
      ...
      <div>
        ...
        {post.postInfo.community === "" ? (
          <button
            className="bg-gray-200 w-25 h-10 text-l mt-2 text-gray-500 cursor-pointer rounded-lg"
            onClick={() => setshowSelectCommunity()}
          >
            +添加社区
          </button>
        ) : (
          <div
            className="inline-block h-10 text-l mt-2 bg-gray-200 text-gray-500 cursor-pointer rounded-lg"
            onClick={() => cancelSelectCommunity()}
          >
      ...
      <div className="flex mt-4 absolute bottom-10 right-5">
        <button
          className="bg-orange-400 hover:bg-orange-500 text-white font-bold py-2 px-10 rounded-lg cursor-pointer"
          onClick={handlePost}
          disabled={locked}
        >
          发布
