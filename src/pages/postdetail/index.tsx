  ...
  return (
    <div className="flex flex-col relative bg-white min-h-screen">
      <main className="flex-grow">
          ...
          {postInfo?.image_url && (
            <div className="flex justify-center items-center w-100  bg-gray-100">
              <img
                ...
                alt={postInfo?.image_url}
              />
            </div>
          )}
          <p className="text-2xl text-left w-full">{postInfo?.content}</p>
        </article>
        {postInfo?.community_category && (
          <div className="h-10 ml-40 mt-10">
            <div className="flex items-center p-2">
              <span className="bg-gray-100 p-2 rounded-2xl">
                # {postInfo?.community_category}
              </span>
            </div>
          </div>
        ...
        <CommentBox
          post_id={post_id}
          refreshCommentList={handleRefreshCommentListKey}
        />
        <CommentList
          post_id={post_id}
          refreshCommentList={CommentListRefresh}
        />
      </main>
      <Footer />
    </div>
