  ...
  const startTimer = () => {
    ...
    intervalRef.current = setInterval(() => {
      setCurrentIndex((prev) =>
        prev === reviewList.length - 1 ? 0 : prev + 1,
      );
    }, 5000);
    ...
    <div className=" text-white font-pixel ">
      <div className="flex flex-col relative">
        ...
        <div className="flex items-center justify-center w-auto mt-10">
          <div
            className="flex items-center justify-center w-300 h-100 relative overflow-hidden"
            onMouseEnter={stopTimer}
            onMouseLeave={startTimer}
          >
            {/* 箭头 */}
            <div
              className="absolute w-15 h-30 left-0 top-1/2 cursor-pointer bg-black/30 flex items-center justify-center rounded-md -translate-y-1/2 z-10"
              onClick={handlePrev}
            >
              <img src={arrowleft} alt="left" />
            </div>
            <div
              className="absolute w-15 h-30 right-0 top-1/2 cursor-pointer bg-black/30 flex items-center justify-center rounded-md -translate-y-1/2 z-10"
              onClick={handleNext}
            >
                ...
                return (
                  ...
                  >
                    ...
                    <img
                      src={item?.image_url}
                      alt="Game Review"
                      className=" w-full absolute opacity-50 "
                    />
                    <div className="h-1/2 " />
                    <div className="absolute w-full h-1/2 bg-gradient-to-t from-black to-transparent opacity-50 flex flex-wrap z-5"></div>
                    <div className="absolute w-full h-1/2  flex flex-wrap z-10">
                      ...
                      {/* 评分 */}
                      <span
                        className={`game-review  text-6xl right-0 ml-auto mr-8 absolute ${getReviewColor(item.review)}`}
                      >
                        {item.review}
                      </span>
                      {/* 副标题 */}
