...
export default function CommentBox({
  ...
  async function handlePostComment() {
    ...
    const res = await addComment({
      ...
      user_avatar: user.userInfo.avatar_url,
      author: user.userInfo.name,
    });
    if (res) {
      toast("评论成功");
    ...
    <div className=" flex flex-col  mx-40 mt-10">
      <div className="flex ">
        ...
        <textarea
          placeholder="评论"
          maxLength={100}
          className="focus:bg-white  border-2 border-gray-200 bg-gray-300 rounded-lg pl-2 w-full pt-2 resize-none "
          rows={3}
          ref={commentRef}
          onInput={handleCommentNum}
          onFocus={handleIsClick}
        ></textarea>
      </div>
      {isOpen && (
        <span className="flex items-center justify-center pl-10 mt-2 text-gray-500">
          {commentNum}/100
          <button
            className="ml-auto w-20 mt-1 bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-4 cursor-pointer rounded-lg"
      ...
      <div className="flex mt-2">
        <div
          className={`cursor-pointer  items-center flex ml-10 bg-gray-200 p-2 rounded-full ${liked ? "text-white bg-red-400" : "text-gray-500"} ${isLoading ? "pointer-events-none opacity-50" : ""}`}
          onClick={() => handleLike()}
        >
